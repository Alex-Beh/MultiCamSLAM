...
void FrontEnd::insertKeyFrame(){
    ...
    VLOG(0)<<"Inserting Keyframe... " << endl; 
    // VLOG(1)<<"INSERTING KEYFRAME...: "<<currentFrame->frameId<<endl;
    unique_lock<mutex> lock(mMutexPose);
...
//            points.push_back( Point3f( l.at<double>(0,0) , l.at<double>(1,0) , l.at<double>(2,0)));
        ...
        if(currentFrame->frameId == 0){
            ...
            if(inter_matches.size() > 50){
                ...
                if(num_triangulated > 50)
                {
                    ...
                    // VLOG(1)<<"fraction of Triangulation/number of matches "<<(float)num_triangulated/inter_matches.size();
                    // VLOG(1)<<"fraction of Triangulation/pose inliers "<<(float)num_triangulated/num_initial_inliers;
...
void FrontEnd::OptimizePose(vector<DMatch> matches, vector<int> lids, vector<bool>& inliers, transformation_t nonlinear_transformation, Mat& poseOut ){
    ...
    //
    VLOG(2)<<"Total time taken for optimization in pose estimation: "<<duration.count()<<endl;
...
int FrontEnd::absolutePoseFromGP3P(MultiCameraFrame* lf_prev, MultiCameraFrame* lf_cur,
    ...
    // VLOG(1) << "Ransac Inliers: "<<ransac.inliers_.size() << endl;
    // VLOG(1) << "Optimization inliers : "<< num_inliers << endl;
...
bool FrontEnd::trackFrame(){
    ...
    MultiCameraFrame* prev_KF = lfFrames.back();
   ...
   // else
    ...
    auto stopT = high_resolution_clock::now();
    auto duration = duration_cast<milliseconds>(stopT - startT);
    VLOG(1)<<"Time taken to search local Map and optimize :"<<duration.count()<<endl;
...
//                        cv::circle(all,p2+Point2f(ci * camconfig_.im_size_.width, 0),3,Scalar(0,0,130), 2);
//                    }
//                }
//            }
...
//                cv::arrowedLine(all, p1+ Point2f(camIndCur * camconfig_.im_size_.width, 0), p2+Point2f(camIndCur * camconfig_.im_size_.width, 0), Scalar(0,170,20), 2);
//            }
//            else{
//                cv::circle(all,p2+Point2f(camIndCur * camconfig_.im_size_.width, 0),3,Scalar(0,0,130), 2);
